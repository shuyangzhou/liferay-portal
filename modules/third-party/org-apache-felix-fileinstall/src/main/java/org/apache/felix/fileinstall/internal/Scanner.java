...
public class Scanner implements Closeable {
    ...
    public Set<File> scan(boolean reportImmediately)
    {
        ...
        Collections.shuffle(list, scannerRandomSource);
    ...
    protected static Random scannerRandomSource;

    static {
        Long scannerShuffleSeed = null;

        try {
            scannerShuffleSeed = Long.valueOf(System.getProperty("osgi.shuffle.seed"));

            scannerRandomSource = new Random(scannerShuffleSeed);
        }
        catch(NumberFormatException nfe) {
            scannerRandomSource = new Random();
        }

        String testBuildTimestamp = System.getProperty("osgi.shuffle.timestamp");
        String logOutputPath = System.getProperty("osgi.shuffle.output.log.path");

        if ((testBuildTimestamp == null) || (logOutputPath == null)) {
            System.out.println("Felix OSGi shuffle logging failed, configuration not supplied. Shuffling still enabled");
        }
        else {
            Writer writer = null;
            ...
            try {
                ...
                if (scannerShuffleSeed != null) {
                    System.out.println("Felix FileInstall started with seed: " + scannerShuffleSeed + "\n");
                }
                else {
                    System.out.println("Felix FileInstall started with random seed\n");
                }

                writer.write("Felix patch applied " + testBuildTimestamp);
            }
