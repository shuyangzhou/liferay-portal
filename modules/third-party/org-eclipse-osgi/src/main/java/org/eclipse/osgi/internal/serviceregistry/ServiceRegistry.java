	...
	void addServiceRegistration(BundleContextImpl context, ServiceRegistrationImpl<?> registration) {
		...
		for (String clazz : registration.getClasses()) {
			while (true) {
				...
				List<ServiceRegistrationImpl<?>> newServices = new ArrayList<>(
					services == null ? Collections.emptyList() : services);

				insertIndex = -Collections.binarySearch(newServices, registration) - 1;

				newServices.add(insertIndex, registration);

				if (services == null) {
					if (publishedServicesByClass.putIfAbsent(clazz, newServices) == null) {
						break;
					}
				}
				else if (publishedServicesByClass.replace(clazz, services, newServices)) {
					break;
				}
			}
		}
	...
	void modifyServiceRegistration(BundleContextImpl context, ServiceRegistrationImpl<?> registration) {
		...
		for (String clazz : registration.getClasses()) {
			while (true) {
				...
				List<ServiceRegistrationImpl<?>> newServices = new ArrayList<>(services);

				newServices.remove(registration);

				// The list is sorted, so we must find the proper location to insert
				insertIndex = -Collections.binarySearch(newServices, registration) - 1;
				newServices.add(insertIndex, registration);

				if (publishedServicesByClass.replace(clazz, services, newServices)) {
					break;
				}
			}
		}
		...
		for (String clazz : registration.getClasses()) {
			while (true) {
				...
				if (newServices.isEmpty()) {
					if (publishedServicesByClass.remove(clazz, services)) {
						break;
					}
				}
				else if (publishedServicesByClass.replace(clazz, services, newServices)) {
