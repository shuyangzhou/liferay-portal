...
public class ServiceRegistrationImpl<S> implements ServiceRegistration<S>, Comparable<ServiceRegistrationImpl<?>> {
	...
	private void _printBefore(ServiceUse<S> use) {
		...
		System.out.println(Thread.currentThread().getName() + "-" + count + "-before-for-" + properties);
	}

	private void _printAfter(ServiceUse<S> use) {
		if (!(use instanceof ServiceFactoryUse)) {
			return;
		}
		...
		System.out.println(Thread.currentThread().getName() + "-" + count + "-after-for-" + properties);
	}
