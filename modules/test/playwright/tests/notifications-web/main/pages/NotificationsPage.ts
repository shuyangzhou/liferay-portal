...
export class NotificationsPage {
	...
	async goto() {
		...
		// on a menu opening or on what it holds. The address carries the back
		// url the dropdown's own link carries, because the back button the
		// portlet renders from it is asserted here.

		await gotoWithRetry(
			this.page,
			`${PORTLET_URLS.notifications}&_com_liferay_notifications_web_portlet_NotificationsPortlet_backURL=%2F`
		);
	}
}
