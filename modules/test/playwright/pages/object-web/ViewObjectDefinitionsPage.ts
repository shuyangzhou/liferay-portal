...
export class ViewObjectDefinitionsPage {
	...
	async searchObjectDefinition(objectDefinitionLabel: string) {
		...
		// term typed into a remounting input is replaced by the one the address
		// still holds and the Enter that follows asks for nothing. Ask again
		// until the list answers for THIS term: fifteen seconds an ask, which is
		// the configured expect timeout, and forty five in all, which leaves
		// half the test's budget for the reach that follows.
			...
			const objectDefinitionsResponsePromise = this.page.waitForResponse(
				(response) => {
					...
					return (
						...
						url.includes(
							'search=' +
								encodeURIComponent(objectDefinitionLabel)
						) &&
						response.request().method() === 'GET'
					);
				},
