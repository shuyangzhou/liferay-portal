...
export class ViewObjectDefinitionsPage {
	...
	async changeObjectActivateStatus(objectDefinitionName: string) {
		...
		// The save answers with its PUT and then schedules a page reload a
		// second after its toast, and the toast of a previous save can still
		// be on screen, so neither the toast nor the click says the save
		// finished. Wait for the save's own answer, then consume the reload
		// it schedules, so the caller's next navigation cannot collide with
		// it. Ten seconds is generous headroom for that timer and fails loud,
		// rather than quietly at the default half minute.
