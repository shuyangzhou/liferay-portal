...
public class BackgroundTaskLocalServiceImpl
	...
	public BackgroundTask amendBackgroundTask(
		...
		if (status == BackgroundTaskConstants.STATUS_SUCCESSFUL) {
			System.out.println("########## Thread id : " + Thread.currentThread().getId() + ", Check fetch before final update " + backgroundTask);

			if (backgroundTask.getMvccVersion() == 0) {
				...
				System.out.println("@@@@@@ Thread id : " + Thread.currentThread().getId() + ", Direct loading from entity cache " + backgroundTask2);
				...
				System.out.println("@@@@@@ Thread id : " + Thread.currentThread().getId() + ", Direct loading from entity cache after clearing thread local cache " + backgroundTask2);

				EntityCacheUtil.clearCache();

				backgroundTask2 = backgroundTaskPersistence.fetchByPrimaryKey(backgroundTaskId);

				System.out.println("@@@@@@ Thread id : " + Thread.currentThread().getId() + ", Persistence reloading after clearing entity cache " + backgroundTask2);
			}
		}
