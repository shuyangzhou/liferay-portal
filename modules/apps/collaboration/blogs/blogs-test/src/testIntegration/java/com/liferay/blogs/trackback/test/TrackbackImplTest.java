	...
	public void testAddTrackback() throws Exception {
		...
		Collection<ServiceReference<MVCActionCommand>> serviceReferences =
			bundleContext.getServiceReferences(
				MVCActionCommand.class, "(mvc.command.name=/blogs/trackback)");
		...
		ServiceReference<MVCActionCommand> serviceReference = iterator.next();

		MVCActionCommand mvcActionCommand = bundleContext.getService(
			serviceReference);

		Assert.assertNotNull(mvcActionCommand);
		...
		ReflectionTestUtil.invoke(
			mvcActionCommand, "_addTrackback",
			new Class<?>[] {
