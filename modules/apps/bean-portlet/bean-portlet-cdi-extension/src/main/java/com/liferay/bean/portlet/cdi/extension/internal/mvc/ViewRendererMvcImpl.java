...
public class ViewRendererMvcImpl implements ViewRenderer {
	...
	public ViewRendererMvcImpl(BeanManager beanManager, boolean importsMvcPackage, boolean importsMvcBindingPackage) {
	...
	public void render(
		...
		if (viewName == null) {
			if (portletRequest.getParameter(REDIRECTED_VIEW) != null) {
		...
		if (_importsMvcBindingPackage) {
			MutableBindingResult mutableBindingResult = BeanUtil.getMutableBindingResult(_beanManager);
			...
			if ((mutableBindingResult != null) && !mutableBindingResult.isConsulted()) {
				...
				for (ParamError paramError : allErrors) {
					if (_log.isWarnEnabled()) {
						_log.warn(StringBundler.concat("BindingResult error not processed for ", paramError.getParamName(), ": ", paramError.getMessage()));
					}
				}
			}
		}
	...
	private static List<ViewEngine> _getViewEngines(BeanManager beanManager) {
		...
		return (List<ViewEngine>)beanManager.getReference(
			bean, _viewEnginesTypeLiteral.getType(),
			beanManager.createCreationalContext(bean));
	}
