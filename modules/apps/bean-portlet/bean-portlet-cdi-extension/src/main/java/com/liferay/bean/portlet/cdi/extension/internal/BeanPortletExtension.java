 ...
 * @author Raymond Augé
 */
// TODO: Rename class to CDIBeanPortletExtension
public class BeanPortletExtension implements Extension {
	...
	public void step4ApplicationScopedInitializedAsync(
			...
			new BeanPortletMethodInvoker() {
				...
				private void _invokePortletBeanMethods(
						...
						try {
							...
							if (methodType == MethodType.ACTION) {
								...
								if (Validator.isNull(beanMethodResourceID) ||
									...
									String contentType =
										methodType.getContentType(method);
									...
									String characterEncoding =
										methodType.getCharacterEncoding(method);
					...
					if (portletResponse instanceof RenderResponse ||
						...
						_viewRenderer.render(portletRequest, (MimeResponse)portletResponse, portletConfig);
					}
				}
		...
		// MVC
		Bean<?> bean = beanManager.resolve(beanManager.getBeans(ViewRenderer.class));
		...
		if (bean == null) {
			...
			_viewRenderer = (ViewRenderer)beanManager.getReference(bean, bean.getBeanClass(), beanManager.createCreationalContext(bean));
		}
	}
