 ...
 * @author Neil Griffin
 */
public class ControllerInterceptor extends BeanMethodInterceptor {
	...
	public Object invoke(Object... args) throws ReflectiveOperationException {
		...
		if ((methodType == MethodType.RENDER) ||
		...
		BeanMethod beanMethod = getWrapped();

		ManagedBean<?> managedBean = beanMethod.getManagedBean();

		_applicationEventPublisher.publishEvent(
			new BeforeControllerEventImpl(
				_target,
				new ResourceInfoImpl(
					managedBean.getBeanType(), beanMethod.getMethod()),
				new UriInfoImpl()));
