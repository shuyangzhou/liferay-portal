...
create index IX_3D016038 on KaleoAction (kaleoClassName[$COLUMN_LENGTH:200$], kaleoClassPK, companyId, executionType[$COLUMN_LENGTH:20$]);
...
create index IX_241376B4 on KaleoDefinition (companyId, active_, scope[$COLUMN_LENGTH:75$]);
...
create index IX_6D12641D on KaleoInstance (companyId, userId, kaleoInstanceId);
create index IX_ACF16238 on KaleoInstance (completed, kaleoDefinitionId);
create index IX_3DA1A5AC on KaleoInstance (completed, kaleoDefinitionVersionId);
...
create index IX_E66A153A on KaleoLog (kaleoInstanceTokenId, type_[$COLUMN_LENGTH:50$], kaleoClassName[$COLUMN_LENGTH:200$], kaleoClassPK);
create index IX_B0CDCA38 on KaleoLog (kaleoTaskInstanceTokenId);

create index IX_4B1D16B4 on KaleoNode (kaleoDefinitionVersionId, companyId);
...
create index IX_CF204C89 on KaleoTaskAssignmentInstance (assigneeClassName[$COLUMN_LENGTH:200$], kaleoTaskInstanceTokenId);
create index IX_38A47B17 on KaleoTaskAssignmentInstance (assigneeClassPK, groupId);
...
create index IX_F904A89A on KaleoTimerInstanceToken (kaleoInstanceTokenId, completed, blocking);
