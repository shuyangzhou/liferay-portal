...
public class LiferayServiceExtender extends AbstractExtender {
	...
	private class LiferayServiceExtension implements Extension {
		...
		public void start() throws Exception {
			...
			_registerService(
				extendeeBundleContext, DataSource.class, dataSource,
				"data.source.bundle.symbolic.name");
			...
			_registerService(
				extendeeBundleContext, SessionFactory.class, sessionFactory,
				"session.factory.bundle.symbolic.name");

			TransactionExecutor transactionExecutor = _getTransactionExecutor(
				dataSource, sessionFactoryImplementor);

			_registerService(
				extendeeBundleContext, TransactionExecutor.class,
		...
		private <T> void _registerService(
			BundleContext bundleContext, Class<T> serviceClass, T service,
			String propertyName) {
			...
			if (propertyName != null) {
				...
				properties.put(propertyName, _extendeeBundle.getSymbolicName());
			}

			_serviceRegistrations.add(
				bundleContext.registerService(
					serviceClass, service, properties));
		}
