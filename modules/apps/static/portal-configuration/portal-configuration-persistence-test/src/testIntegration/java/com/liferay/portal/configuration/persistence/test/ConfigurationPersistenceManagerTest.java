...
public class ConfigurationPersistenceManagerTest {
	...
	@Test
	public void testConfigurationOverwrite() throws IOException {

		// Overwrite non-exist configuration

		String testPid = "testPid";

		_persistenceManager.delete(testPid);

		Map<String, String> overwriteProperties = HashMapBuilder.put(
			"a", "A"
		).put(
			"b", "B"
		).build();

		try (Closeable closeable = _reloadWithOverwriteProperties(
				testPid, overwriteProperties)) {
			...
			// Runtime creation should not affect overwrite
		...
		try (Closeable closeable = _reloadWithOverwriteProperties(
			...
			// Runtime update should not affect overwrite
	...
	private Closeable _reloadWithOverwriteProperties(
		String pid, Map<String, String> properties) {

		Map<String, Map<String, String>> innerMap =
			ReflectionTestUtil.getFieldValue(
				ConfigurationOverwritePropertiesUtil.
