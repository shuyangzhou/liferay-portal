...
public class CTCollectionLocalServiceTest {
	...
	public void testCheckConflictsWithJournalArticles() throws Exception {
		...
		conflictInfos.sort(
			Comparator.comparing(ConflictInfo::getProductionPrimaryKey));
		...
		Assert.assertEquals(
			...
			conflictInfo.getProductionPrimaryKey());
		Assert.assertEquals(
			ctJournalArticle1.getPrimaryKey(), conflictInfo.getCTPrimaryKey());
		...
		Assert.assertEquals(
			...
			conflictInfo.getProductionPrimaryKey());
		Assert.assertEquals(
			ctJournalArticle2.getPrimaryKey(), conflictInfo.getCTPrimaryKey());
	}
	...
	public void testCheckConflictsWithJournalFolders() throws Exception {
		...
		Assert.assertEquals(
			...
			conflictInfo.getProductionPrimaryKey());
		Assert.assertEquals(
			ctJournalFolder.getPrimaryKey(), conflictInfo.getCTPrimaryKey());
	}
