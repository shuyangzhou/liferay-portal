...
public class MessageBusCTTest {
	...
	public void testSendMessage() {
		...
		try (SafeClosable safeClosable1 =
			...
			List<CTMessage> ctMessages = _ctMessageLocalService.getCTMessages(
				_ctCollection.getCtCollectionId());

			Assert.assertSame(ctMessages.toString(), 1, ctMessages.size());

			CTMessage ctMessage = ctMessages.get(0);

			Assert.assertEquals(
				_ctCollection.getCompanyId(), ctMessage.getCompanyId());
			Assert.assertEquals(
				_ctCollection.getUserId(), ctMessage.getUserId());
			Assert.assertEquals(
				_TEST_DESTINATION_NAME, ctMessage.getDestinationName());

			Message deSerializedMessage = (Message)_jsonFactory.deserialize(
