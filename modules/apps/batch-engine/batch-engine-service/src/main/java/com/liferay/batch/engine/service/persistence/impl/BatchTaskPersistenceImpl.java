...
public class BatchTaskPersistenceImpl
	...
	public List<BatchTask> findByUuid(
		...
		List<BatchTask> list = (List<BatchTask>)finderCache.getResult(
	...
	public BatchTask updateImpl(BatchTask batchTask) {
		...
		try {
			...
			if (batchTask.isNew()) {
				...
				batchTask = (BatchTask)session.merge(batchTask);
			}
		}
		catch (Exception e) {
			throw processException(e);
	...
	static {
		try {
			Class.forName(BatchPersistenceConstants.class.getName());
		}
