...
public class ObjectActionAttachmentDownloadTest extends BaseWebServerTestCase {
	...
	public void testRunsForItsOwnAttachment() throws Exception {
		...
		MockHttpServletResponse mockHttpServletResponse = _download(
			_getFileEntry(objectEntry), objectEntry.getExternalReferenceCode());
	...
	public void testRunsOncePerDownload() throws Exception {
		...
		_download(fileEntry, objectEntry.getExternalReferenceCode());

		Assert.assertEquals(1, _executionCount.get());

		// The engine runs an action once per set of object entry IDs held in a
		// thread local, and a request filter clears it between requests. This
		// harness calls the servlet directly on one thread, so clear it here to
		// make the second download a second request rather than a repeat of the
		// first.
	...
	public void testSkipsAnotherObjectEntrysAttachment() throws Exception {
		...
		FileEntry fileEntry = _getFileEntry(objectEntry1);

		// The file belongs to the first entry and the request names the second
	...
	private MockHttpServletResponse _download(
			FileEntry fileEntry, String objectEntryExternalReferenceCode)
		throws Exception {
