...
String jobProperties = StringPool.BLANK;

String jobType = ParamUtil.getString(request, "jobType");

if (dispatchTrigger != null) {
	dispatchTriggerId = dispatchTrigger.getDispatchTriggerId();
	jobProperties = dispatchTrigger.getJobProperties();
	jobType = dispatchTrigger.getJobType();
}
%>
...
<div class="closed container-fluid-1280" id="<portlet:namespace />editDispatchTriggerId">
	<div class="container main-content-body sheet">
		...
		<aui:form action="<%= editDispatchTriggerActionURL %>" cssClass="container-fluid-1280" method="post" name="fm">
			...
			<aui:input name="jobType" type="hidden" value="<%= jobType %>" />
			<aui:input name="jobProperties" type="hidden" />
			...
			<div class="lfr-form-content">
				...
				<div id="<portlet:namespace />jobPropertiesEditor"></div>
...
<aui:script use="aui-ace-editor,liferay-xml-formatter">
	...
	var contentEditor = new A.AceEditor({
		boundingBox: '#<portlet:namespace />jobPropertiesEditor',
		height: 600,
		mode: 'xml',
		tabSize: 4,
		width: '100%',
	...
	var content = xmlFormatter.format('<%= HtmlUtil.escapeJS(jobProperties) %>');
	...
	Liferay.on('<portlet:namespace />saveTrigger', function (event) {
		...
		form['<portlet:namespace />jobProperties'].value = contentEditor.get(
			STR_VALUE
		);
