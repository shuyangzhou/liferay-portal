...
package com.liferay.service.builder.test.service.base;
...
import com.liferay.service.builder.test.model.impl.VersionedEntryImpl;
import com.liferay.service.builder.test.service.VersionedEntryLocalService;
import com.liferay.service.builder.test.service.persistence.VersionedEntryPersistence;
 ...
 *
 * <p>
 * This implementation exists only as a container for the default service methods generated by ServiceBuilder. All custom service methods should be put in {@link com.liferay.service.builder.test.service.impl.VersionedEntryLocalServiceImpl}.
 * </p>
 *
...
public abstract class VersionedEntryLocalServiceBaseImpl
	 ...
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never modify or reference this class directly. Always use {@link com.liferay.service.builder.test.service.VersionedEntryLocalServiceUtil} to access the versioned entry local service.
	 */
	...
	public VersionedEntry create() {
		...
		VersionedEntry versionedEntry = versionedEntryPersistence.create(primaryKey);

		versionedEntry.setHeadId(primaryKey);
	...
	public VersionedEntry deleteVersionedEntry(long versionedEntryId)
		...
		return versionedEntryPersistence.remove(versionedEntryId);
	}
	...
	public VersionedEntry deleteVersionedEntry(VersionedEntry versionedEntry) {
		return versionedEntryPersistence.remove(versionedEntry);
	}
	...
	@Indexable(type = IndexableType.REINDEX)
	@Override
	public VersionedEntry updateVersionedEntry(VersionedEntry versionedEntry) {
		return update(versionedEntry);
	}
	...
	public void afterPropertiesSet() {
		persistedModelLocalServiceRegistry.register("com.liferay.service.builder.test.model.VersionedEntry",
			versionedEntryLocalService);
	}

	public void destroy() {
		persistedModelLocalServiceRegistry.unregister(
			"com.liferay.service.builder.test.model.VersionedEntry");
	}

	@Override
	public List<VersionedEntryVersion> delete(VersionedEntry versionedEntry) {
		if (versionedEntry.isDraft()) {
			versionedEntryPersistence.remove(versionedEntry);

			return Collections.emptyList();
		}

		_deleteDraft(versionedEntry);

		List<VersionedEntryVersion> versionedEntryVersions = getVersions(versionedEntry);
		...
		versionedEntryLocalService.deleteVersionedEntry(versionedEntry);

		return versionedEntryVersions;
	}
	...
	public VersionedEntryVersion deleteVersion(
		...
		if (latestVersionedEntryVersion.getVersion() == versionedEntryVersion.getVersion()) {
			throw new IllegalArgumentException("Cannot delete latest version " +
				...
				", revert to go back to a previous version");
		}

		return versionedEntryVersionPersistence.remove(versionedEntryVersion);
	}
	...
	public VersionedEntry getDraft(VersionedEntry versionedEntry) {
		...
		if (draftVersionedEntry == null) {
			draftVersionedEntry = _createDraft(versionedEntry);
		}
	...
	@Indexable(type = IndexableType.REINDEX)
	@Override
	public VersionedEntry publish(VersionedEntry versionedEntry)
		throws PortalException {
		if (!versionedEntry.isDraft()) {
		...
		if (versionedEntry.getHeadId() == versionedEntry.getPrimaryKey()) {
			return _publishVersion(versionedEntry,
				versionedEntry.toVersionModel());
		}

		VersionedEntry publishedVersionedEntry = versionedEntryPersistence.fetchByPrimaryKey(versionedEntry.getHeadId());

		publishedVersionedEntry.setContent(versionedEntry.getContent());

		publishedVersionedEntry = _publishVersion(publishedVersionedEntry,
				publishedVersionedEntry.toVersionModel());
	...
	@Indexable(type = IndexableType.REINDEX)
	@Override
	public VersionedEntry revert(VersionedEntry versionedEntry, int version)
		throws PortalException {
		VersionedEntryVersion versionedEntryVersion = getVersion(versionedEntry,
				version);

		VersionedEntry currentVersionedEntry = null;

		if (versionedEntry.isDraft()) {
			currentVersionedEntry = versionedEntryPersistence.findByPrimaryKey(versionedEntry.getHeadId());

			versionedEntryPersistence.remove(versionedEntry);
		}
		else {
			currentVersionedEntry = versionedEntry;

			_deleteDraft(versionedEntry);
		}

		VersionedEntry previousVersionedEntry = versionedEntryVersion.toVersionedModel();

		previousVersionedEntry.setMvccVersion(currentVersionedEntry.getMvccVersion());

		return _publishVersion(previousVersionedEntry,
			previousVersionedEntry.toVersionModel());
	}

	@Indexable(type = IndexableType.REINDEX)
	@Override
	public VersionedEntry update(VersionedEntry versionedEntry) {
		if (versionedEntry.isDraft()) {
			return versionedEntryPersistence.update(versionedEntry);
		}
		...
		if (draftVersionedEntry != null) {
			throw new IllegalArgumentException("Draft already exists for " +
				versionedEntry.getPrimaryKey());
		}

		draftVersionedEntry = _createDraft(versionedEntry);

		return versionedEntryPersistence.update(draftVersionedEntry);
	}

	private VersionedEntry _createDraft(VersionedEntry versionedEntry) {
		VersionedEntryImpl draftVersionedEntry = new VersionedEntryImpl();

		draftVersionedEntry.setNew(true);

		draftVersionedEntry.setMvccVersion(0);
		draftVersionedEntry.setPrimaryKey(counterLocalService.increment(
				VersionedEntry.class.getName()));
		draftVersionedEntry.setContent(versionedEntry.getContent());
		draftVersionedEntry.setHeadId(versionedEntry.getPrimaryKey());
	...
	private void _deleteDraft(VersionedEntry versionedEntry) {
		VersionedEntry draftVersionedEntry = versionedEntryPersistence.fetchByHeadId(versionedEntry.getPrimaryKey());

		if (draftVersionedEntry != null) {
			versionedEntryLocalService.deleteVersionedEntry(draftVersionedEntry);
		}
	}

	private VersionedEntry _publishVersion(VersionedEntry versionedEntry,
		VersionedEntryVersion versionedEntryVersion) {
		int version = 1;

		VersionedEntryVersion latestVersionedEntryVersion = fetchLatestVersion(versionedEntry);

		if (latestVersionedEntryVersion != null) {
			version = latestVersionedEntryVersion.getVersion() + 1;
		}

		versionedEntryVersion.setNew(true);
		versionedEntryVersion.setPrimaryKey(counterLocalService.increment(
		...
		versionedEntry.setHeadId(-versionedEntry.getPrimaryKey());
