...
public class VersionedEntryPersistenceImpl extends BasePersistenceImpl<VersionedEntry>
	 ...
	 *
	 * @param entryId the primary key of the current versioned entry
	 * @param groupId the group ID
	 * @param orderByComparator the comparator to order the set by (optionally <code>null</code>)
	 ...
	 */
	@Override
	public VersionedEntry[] findByGroupId_PrevAndNext(long entryId,
		long groupId, OrderByComparator<VersionedEntry> orderByComparator)
		throws NoSuchVersionedEntryException {
		VersionedEntry versionedEntry = findByPrimaryKey(entryId);
		...
		if (result == null) {
			...
			try {
				...
				if (list.isEmpty()) {
					...
					if ((versionedEntry.getHeadId() != headId)) {
						finderCache.putResult(FINDER_PATH_FETCH_BY_HEADID,
							finderArgs, versionedEntry);
					}
				}
			}
			catch (Exception e) {
	 ...
	 *
	 * @param entryId the primary key for the new versioned entry
	 * @return the new versioned entry
	 */
	@Override
	public VersionedEntry create(long entryId) {
		VersionedEntry versionedEntry = new VersionedEntryImpl();

		versionedEntry.setNew(true);
		versionedEntry.setPrimaryKey(entryId);
	 ...
	 */
	@Override
	public VersionedEntry remove(long entryId)
		throws NoSuchVersionedEntryException {
	 ...
	 */
	@Override
	public VersionedEntry findByPrimaryKey(long entryId)
		throws NoSuchVersionedEntryException {
	 ...
	 */
	@Override
	public VersionedEntry fetchByPrimaryKey(long entryId) {
	...
	private static final String _SQL_SELECT_VERSIONEDENTRY_WHERE_PKS_IN = "SELECT versionedEntry FROM VersionedEntry versionedEntry WHERE entryId IN (";
