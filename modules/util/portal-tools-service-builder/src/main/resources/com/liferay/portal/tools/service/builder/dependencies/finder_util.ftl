...
public class ${entity.name}FinderUtil {
	...
	public static ${entity.name}Finder getFinder() {
		if (_finder == null) {
			<#if pluginName != "">
				_finder = (${entity.name}Finder)PortletBeanLocatorUtil.locate(${servicePackagePath}.service.ClpSerializer.getServletContextName(), ${entity.name}Finder.class.getName());
			<#else>
