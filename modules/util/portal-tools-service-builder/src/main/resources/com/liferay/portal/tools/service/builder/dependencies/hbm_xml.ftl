<#list entities as entity>
	<import class="${servicePackagePath}.model.${entity.name}" />
</#list>
	...
	<#if entity.hasColumns()>
		...
		>
			<#if entity.hasCompoundPK()>
				<composite-id name="primaryKey" class="${servicePackagePath}.service.persistence.${entity.name}PK">
			...
			<#list entity.columnList as column>
				...
				<#if !column.isPrimary() && !column.isCollection() && !ejbName && ((column.type != "Blob") || ((column.type == "Blob") && !column.lazy)) && (column.name != "mvccVersion")>
					...
					<#if (serviceBuilder.getSqlType(servicePackagePath + ".model." + entity.getName(), column.getName(), column.getType()) == "CLOB") && (column.type != "Map")>
				...
				<#if (column.type == "Blob") && column.lazy>
					<one-to-one name="${column.name}BlobModel" access="com.liferay.portal.dao.orm.hibernate.PrivatePropertyAccessor" class="${servicePackagePath}.model.${entity.name}${column.methodName}BlobModel" cascade="save-update" outer-join="false" constrained="true" />
				</#if>
			</#list>
		</class>
		...
		<#list entity.blobList as blobColumn>
			<#if blobColumn.lazy>
				<class name="${servicePackagePath}.model.${entity.name}${blobColumn.methodName}BlobModel" table="${entity.table}" lazy="true"
					<#if entity.isDynamicUpdateEnabled()>
						dynamic-update="true"
					</#if>
				>
