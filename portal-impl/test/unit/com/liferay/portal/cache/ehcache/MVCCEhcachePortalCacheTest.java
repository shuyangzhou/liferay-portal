...
public class MVCCEhcachePortalCacheTest {
	...
	public void testMVCCCacheWithAdvice() throws Exception {
		...
		_recordCacheListener.assertPut(_KEY_1, new MockMVCCModel(_VERSION_1));
		...
		_recordCacheListener.assertUpdated(
			_KEY_1, new MockMVCCModel(_VERSION_2));
	...
	private static class RecordCacheListener
		implements CacheListener<String, MVCCModel> {
		...
		public void assertPut(String key, MVCCModel value) {
		...
		public void assertUpdated(String key, MVCCModel value) {
			Assert.assertFalse(_put);
			Assert.assertFalse(_removeAll);
			Assert.assertFalse(_removed);
