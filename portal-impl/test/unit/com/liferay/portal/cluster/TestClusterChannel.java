...
public class TestClusterChannel implements ClusterChannel {
	...
	public void close() {
		...
		final List<Address> view = ListUtil.fromCollection(
			clusterChannels.keySet());

		for (TestClusterChannel clusterChannel : clusterChannels.values()) {
			final ClusterReceiver clusterReceiver =
				clusterChannel.getReceiver();

			new Thread() {

				@Override
				public void run() {
					clusterReceiver.viewAccepted(view);
				}
