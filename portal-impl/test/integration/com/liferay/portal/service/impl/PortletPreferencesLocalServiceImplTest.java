...
public class PortletPreferencesLocalServiceImplTest {
	...
	public void testAddPortletPreferencesConcurrently() throws Exception {
		...
		try (CaptureAppender captureAppender1 =
			...
			int expectedLogs = _THREAD_COUNT - 1;

			if (!dbType.equals(DB.TYPE_MYSQL)) {
				expectedLogs = expectedLogs * 2;
			}

			Assert.assertEquals(expectedLogs, loggingEvents.size());
			...
			for (LoggingEvent loggingEvent : loggingEvents) {
				...
				switch (dbType) {
					case DB.TYPE_DB2:
						if (!message.contains("Batch failure.") &&
							!message.contains(
					...
					case DB.TYPE_ORACLE:
						Assert.assertTrue(
							message,
							message.startsWith("ORA-00001: unique constraint"));
					...
					case DB.TYPE_POSTGRESQL:
						...
						sb.append(_OWNER_ID);
						sb.append(", ");
						sb.append(_OWNER_TYPE);
						sb.append(", ");
						sb.append(_PLID);

						String values = sb.toString();

						Assert.assertTrue(message, message.contains(values));
