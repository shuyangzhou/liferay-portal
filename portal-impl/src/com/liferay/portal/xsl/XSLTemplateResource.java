...
public class XSLTemplateResource implements TemplateResource {
	...
	public boolean equals(Object obj) {
		...
		if (_templateId.equals(xslTemplateResource._templateId) &&
			...
			_equalsForTokens(xslTemplateResource._tokens)) {
	...
	@Override
	public int hashCode() {
		return _templateId.hashCode() * 13 + _script.hashCode() * 11 +
			_xml.hashCode() * 7 + _tokens.hashCode();
	}
	...
	private boolean _equalsForTokens(Map<String, String> tokens) {
		if (_tokens == tokens) {
			return true;
		}

		if (_tokens.size() != tokens.size()) {
			return false;
		}

		Set<Map.Entry<String, String>> entries = _tokens.entrySet();

		for (Map.Entry<String, String> entry : entries) {
			String key = entry.getKey();
			String value1 = entry.getValue();

			String value2 = tokens.get(key);
