...
public class ASMWrapperUtil {
		...
		synchronized (classLoader) {
			...
			try {
				if (asmWrapperClass == null) {
					...
					byte[] classData = generateASMWrapperClassData(
	...
	public static <T> byte[] generateASMWrapperClassData(
		Class<T> wrappedClass, T defaultObject, Object wrapper) {

		String wrappedClassBinaryName = getClassBinaryName(wrappedClass);
		...
		classWriter.visit(
			...
			asmWrapperClassBinaryName, null, getClassBinaryName(Object.class),
		...
		for (Method method : methods) {
			try {
				...
				_generateMethod(
					...
					defaultObjectClassDescription,
					getClassBinaryName(defaultObject.getClass()));
			...
			_generateMethod(
				...
				wrapperClassDescription, getClassBinaryName(wrapperClass));
		}
	...
	private static void _generateMethod(
		...
		for (int i = 0; i < exceptions.length; i++) {
			exceptionsBinaryClassNames[i] = getClassBinaryName(exceptions[i]);
		}

		MethodVisitor methodVisitor = classWriter.visitMethod(
			_getOpCodes(method.getModifiers()), method.getName(),
	...
	private static int _getOpCodes(int modifiers) {
		int ret = 0;
		...
		if (Modifier.isStatic(modifiers)) {
			ret += Opcodes.ACC_STATIC;
		}

		if (Modifier.isSynchronized(modifiers)) {
			ret += Opcodes.ACC_SYNCHRONIZED;
		}
