...
public class GroupImpl extends GroupBaseImpl {
	...
	public List<Group> getDescendants(boolean site) {
		Set<Group> descendantsSet = new LinkedHashSet<Group>();

		for (Group group : getChildren(site)) {
			descendantsSet.add(group);
			descendantsSet.addAll(group.getDescendants(site));
		}

		ArrayList<Group> descendants = new ArrayList<Group>(descendantsSet);
		return descendants;
	}
