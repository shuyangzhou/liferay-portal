...
public class JavaSourceProcessor extends BaseSourceProcessor {
	...
	protected String checkFinalableFields(
			File file, String packagePath, String className, String content)
		throws IOException {
		...
		builder.addSource(new UnsyncStringReader(content));

		com.thoughtworks.qdox.model.JavaClass javaClass =
			builder.getClassByName(
		...
		for (JavaField javaField : javaClass.getFields()) {
			if (javaField.isPrivate() && !javaField.isFinal()) {
				Pattern pattern = Pattern.compile(
					"\\b".concat(javaField.getName()).concat(
						_assignmentPattern));
	...
	protected String format(
		...
		String newContent = checkFinalableFields(
			file, packagePath, className, content);
