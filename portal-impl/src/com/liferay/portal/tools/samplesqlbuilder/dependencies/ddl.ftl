...
<#if (maxDDLRecordSetCount > 0)>
	<#assign ddmStructure = dataFactory.addDDMStructure(groupId, companyId, 0, dataFactory.DDLRecordSetClassName.classNameId)>
	...
	<#list 1..maxDDLRecordSetCount as ddlRecordSetCount>
		<#assign ddlPageURL = "dynamic_data_list_display_" + ddlRecordSetCount>
		<#assign ddlPortletInstanceName = "169_INSTANCE_TEST" + ddlRecordSetCount>
		<#assign publicLayoutsSize = publicLayouts?size>

		<#assign dynamicDataListDisplayLayout = dataFactory.addLayout(publicLayoutsSize + ddlRecordSetCount, "Dynamic Data List Display " + ddlRecordSetCount, "/" + ddlPageURL, "", ddlPortletInstanceName)>

		<#assign publicLayouts = publicLayouts + [dynamicDataListDisplayLayout]>
		...
		<#if (maxDDLRecordCount > 0)>
			<#list 1..maxDDLRecordCount as ddlRecordCount>
				...
				${writerDynamicDataListsCSV.write(ddlPageURL + "," + ddlPortletInstanceName + "," + ddlRecordSet.recordSetId + "," + ddlRecord.recordId + "\n")}
			</#list>
		</#if>
		...
		<#assign portletPreferences = dataFactory.addPortletPreferences(defaultUserId, dynamicDataListDisplayLayout.plid, ddlPortletInstanceName, preferences)>
		...
		${sampleSQLBuilder.insertResourcePermission("169", dynamicDataListDisplayLayout.plid + "_LAYOUT_" + ddlPortletInstanceName)}
	</#list>
</#if>