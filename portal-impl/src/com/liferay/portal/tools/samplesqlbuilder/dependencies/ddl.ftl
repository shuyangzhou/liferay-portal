...
<#if (maxDDLRecordSetCount > 0)>
	...
	<#list 1..maxDDLRecordSetCount as ddlRecordSetCount>
		<#assign ddlPortletInstanceName = dynamicDataListDisplayNamePrefix + ddlRecordSetCount>
		<#assign ddlPageURL = dynamicDataListDisplayURLPrefix + ddlRecordSetCount>
		<#assign plid = dynamicDataListDisplayLayouts[ddlRecordSetCount - 1].plid>
		...
		<#assign portletPreferences = dataFactory.addPortletPreferences(defaultUserId, plid, ddlPortletInstanceName, preferences)>
		...
		${sampleSQLBuilder.insertResourcePermission("169", plid + "_LAYOUT_" + ddlPortletInstanceName)}
	</#list>
</#if>