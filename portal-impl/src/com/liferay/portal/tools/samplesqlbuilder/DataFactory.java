...
public class DataFactory {
	...
	public Set<Long> getAssetCategoryIds(long groupId) {
		...
		Set<Long> assetCategoryIds = new HashSet<Long>();

		for (int i = 0; i < _maxAssetEntryCategoryCount;) {
			int index = _RANDOM.nextInt(assetCategoryCountPerGroup);

			long assetCategoryId = _assetCategories.get(
				startIndex + index).getCategoryId();

			if (assetCategoryIds.add(assetCategoryId)) {
				i++;
			}
		}
	...
	public Set<Long> getAssetTagIds(long groupId) {
		int startIndex = _maxAssetTagCount * ((int)groupId - 1);

		Set<Long> assetTagIds = new HashSet<Long>();

		for (int i = 0; i < _maxAssetEntryTagCount;) {
			int index = _RANDOM.nextInt(_maxAssetTagCount);

			long assetTagId = _assetTags.get(startIndex + index).getTagId();

			if (assetTagIds.add(assetTagId)) {
				i++;
			}
		}
