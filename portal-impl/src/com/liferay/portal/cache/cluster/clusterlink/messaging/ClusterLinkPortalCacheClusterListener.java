...
public class ClusterLinkPortalCacheClusterListener extends BaseMessageListener {
	...
	protected void handlePortalCacheClusterEvent(
		...
		if (portalCacheClusterEventType.equals(
			...
			PortalCacheHelperUtil.removeAll(portalCache, true);
		}
			...
			if (value == null) {
				PortalCacheHelperUtil.remove(portalCache, key, true);
			}
			else {
				PortalCacheHelperUtil.put(
					portalCache, key, value,
					portalCacheClusterEvent.getTimeToLive(), true);
			}
		}
		else {
			PortalCacheHelperUtil.remove(
				portalCache, portalCacheClusterEvent.getElementKey(), true);
		}
	}
