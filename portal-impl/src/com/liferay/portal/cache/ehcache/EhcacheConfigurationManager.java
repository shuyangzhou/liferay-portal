...
import com.liferay.portal.cache.bootstrap.StreamBootstrapLoaderFactory;
import com.liferay.portal.cache.cluster.PortalCacheClusterReplicatorFactory;
		...
		if (factoryConfiguration != null) {
			...
			portalCacheManagerConfiguration.
				addCacheManagerListenerConfiguration(
					EhcacheCacheManagerListenerFactory.class.getName(),
					properties);
		}
	...
	private PortalCacheConfiguration _parseCacheConfiguration(
			...
			if (clusterAware && PropsValues.CLUSTER_LINK_ENABLED) {
				if (PropsValues.EHCACHE_CLUSTER_LINK_REPLICATION_ENABLED) {
					portalCacheConfiguration.setBootstrapLoaderConfiguration(
						StreamBootstrapLoaderFactory.class.getName(),
						properties);
				}
				else {
					properties.put(
					...
					portalCacheConfiguration.setBootstrapLoaderConfiguration(
						EhcacheBootstrapLoaderFactory.class.getName(),
						properties);
				}
			}
		...
		for (CacheEventListenerFactoryConfiguration
			...
			if (fullyQualifiedClassPath.contains(
				...
				if (clusterAware && PropsValues.CLUSTER_LINK_ENABLED) {
					if (PropsValues.EHCACHE_CLUSTER_LINK_REPLICATION_ENABLED) {
						portalCacheConfiguration.addCacheListenerConfiguration(
							PortalCacheClusterReplicatorFactory.class.getName(),
							properties);
					}
					else {
						properties.put(
						...
						portalCacheConfiguration.addCacheListenerConfiguration(
							EhcacheCacheListenerFactory.class.getName(),
							properties);
				...
				portalCacheConfiguration.addCacheListenerConfiguration(
					EhcacheCacheListenerFactory.class.getName(), properties);
			}
		}
