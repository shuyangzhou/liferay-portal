...
public class SerializableObjectWrapperTest {
	...
	public void testSpecialEqualsWithBrokenClassLoader() throws Exception {
		...
		try (CaptureHandler captureHandler =
			...
			SerializableObjectWrapper serializableObjectWrapper1 =
				new SerializableObjectWrapper(new TestSerializable(sameName));
			SerializableObjectWrapper serializableObjectWrapper2 =
				new SerializableObjectWrapper(
					new SpecialTestSerializable(sameName, ""));

			Assert.assertFalse(
				_getDeserializedObject(serializableObjectWrapper1).equals(
					_getDeserializedObject(serializableObjectWrapper2)));

			_assertLogAndClear(logRecords, cnfe);

			Assert.assertFalse(
				_getDeserializedObject(serializableObjectWrapper2).equals(
					_getDeserializedObject(serializableObjectWrapper1)));
