	...
	private void _updatePortletPreferenceValues() throws Exception {
		...
		try (PreparedStatement ps1 = connection.prepareStatement(sb.toString());
			...
			PreparedStatement ps4 =
				AutoBatchPreparedStatementUtil.concurrentAutoBatch(
					connection,
					"update PortletPreferenceValue set readOnly = ?, " +
						"smallValue = ?, largeValue = ? where " +
							"portletPreferenceValueId = ?");
		...
		for (Map.Entry<String, Map.Entry<PreferenceValues, PreferenceValues>>
			...
			for (int i = 0; i < newValues.size(); i++) {
				...
				if (oldSize > i) {
					...
					if (value.length() > smallValueMaxLength) {
						updatePreparedStatement.setString(7, null);
						updatePreparedStatement.setString(8, value);
					}
					else {
						updatePreparedStatement.setString(7, value);
						updatePreparedStatement.setString(8, null);
					}
