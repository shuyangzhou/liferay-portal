...
public class DataGuardTestRule
			...
			while (iterator.hasNext()) {
				...
				for (BaseModel<?> leftoverBaseModel : leftoverBaseModels) {
					try {
						...
						if (_isNoSuchModelException(throwable)) {
							failure = true;
						}
						else {
							throw throwable;
	...
	private boolean _isNoSuchModelException(Throwable throwable) {
		...
		while (throwable != null) {
			...
			throwable = previousThrowable.getCause();

			if (throwable == previousThrowable) {
				break;
			}
		}
