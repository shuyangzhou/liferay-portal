...
public class TransactionalTestRule implements TestRule {
	...
	protected static class TransactionalFrameworkMethod
		...
		public Object invokeExplosively(
			...
			try (Closeable closeable = _installTransactionManager(
					_originBundleSymbolicName, target.getClass())) {
	...
	private static Closeable _installTransactionManager(
			String originBundleSymbolicName, Class<?> testClass)
		...
		Collection<ServiceReference<PortletTransactionManager>>
			serviceReferences = bundleContext.getServiceReferences(
				PortletTransactionManager.class,
		...
		Assert.assertEquals(
			"Expected 1 PortletTransactionManager for " +
				originBundleSymbolicName + ", actually have " +
					serviceReferences,
			1, serviceReferences.size());

		Iterator<ServiceReference<PortletTransactionManager>> iterator =
			serviceReferences.iterator();

		ServiceReference<PortletTransactionManager> serviceReference =
			iterator.next();
