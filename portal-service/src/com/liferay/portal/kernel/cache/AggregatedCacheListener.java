...
public class AggregatedCacheListener <K extends Serializable, V>
	...
	private boolean _shouldDeliver(
		...
		boolean isRemote = _remoteInvokeThreadLocal.get();

		if (isRemote && (cacheListener instanceof CacheReplicator)) {
			return false;
		}

		return (isRemote && cacheListenerScope.isDeliverRemote()) ||
			!isRemote && (cacheListenerScope.isDeliverLocal());
	}

	private static Log _log = LogFactoryUtil.getLog(
		AggregatedCacheListener.class);

	private static ThreadLocal<Boolean> _remoteInvokeThreadLocal =
		new InitialThreadLocal<Boolean>(
			AggregatedCacheListener.class + "._remoteInvokeThreadLocal", false);
	private static ThreadLocal<Boolean> _skipListenerThreadLocal =
		new InitialThreadLocal<Boolean>(
			AggregatedCacheListener.class + "._skipListenerThreadLocal", false);

	private ConcurrentMap<CacheListener<K, V>, CacheListenerScope>
		_cacheListeners =
